...
plot_result <- function(jags_output,
  ...
  if (inherits(jags_output, "ni_gam_decomp") == TRUE) {
    # Dataframes to plot
    total_model_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$total_model_df
    total_model_rate_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$total_model_rate_df
    # Predicted
    total_model_pred_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$mod_output_pred_df
    total_model_pred_rate_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$mod_output_pred_deriv_df


    time_post_component_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$time_post_component_df
    time_post_pred_component_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$time_post_pred_component_df
    time_post_pred_deriv_component_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$time_post_pred_deriv_component_df

    g_h_component_post_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$g_h_component_post_df

    space_time_component_post_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$space_time_component_post_df
    space_time_component_deriv_post_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$space_time_component_deriv_post_df
    space_time_component_pred_post_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$space_time_component_pred_post_df
    space_time_component_pred_deriv_post_df <-
      parameter_estimate(jags_output = jags_output)$output_dataframes$space_time_component_pred_deriv_post_df
    ...
    all_components_CI_pred_plot <- ggplot2::ggplot() +
      ...
      # # Linear Local Component + site specific vertical offset
      # ggplot2::geom_line(
      #   data = jags_output$g_h_component_post,
      #   ggplot2::aes(x = Age * 1000, y = RSL, colour = ID)
      # ) +
      # ggplot2::geom_ribbon(
      #   data = jags_output$g_h_component_post,
