...
The tide gauges undergo a cleaning process within the package. Firstly, certain tide gauges have been flagged by the PSMSL website and we remove these locations. As collection of proxy reconstructions began in approximately 2000 CE, the tide gauge records are given in meters relative to the average over a time window, 2000 - 2018 CE, which corresponds to the variability of 19-year astronomical tides (Pugh and Woodworth 2014). Next, the tide gauge data is averaged over a decade to make it comparable with sedimentation rates associated proxy records sedimentation rates. 

Within the `reslr_load` function, the user has three options to choose from:

1. Provide a list of the preferred tide gauges from the [PSMSL website](https://psmsl.org/data/obtaining/), ensuring spelling, capitalisation and spacing is exactly the same as the website. Note, the package will not work if error in spelling occurs. In addition, certain tide gauges have been flagged by the PSMSL website and are not included in this package and will return an error if selected. This is done by giving a list to the `list_preferred_TGs = c("ARGENTIA)` option in the `reslr_mcmc` function
...
If tide gauge data required update the argument `include_tide_gauge = TRUE`, from this the user has three options as described above. Firstly, provide the list of names for the tide gauges from PSMSL website that the wish to use in the `list_preferred_TGs` option. Secondly, the package find the tide gauge closest to the proxy site `TG_minimum_dist_proxy = TRUE`. Thirdly, the package uses all tide gauges within 1 degree of the proxy site `all_TG_1deg = TRUE`.
...
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% uncertainty interval. In order to view the rate of change plot, the following setting should be used:

```{r,plotigpresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = TRUE}
plot(jags_output.eiv_igp_t,plot_type = "rate_plot")
```
This will print the plot of the rate of change with 95 \% uncertainty intervals. 
...
**Step 3a:** A brief insight into the outputs of the `reslr_output` function can be obtained using:
```{r,printspstout,eval=FALSE}
print(jags_output.ni_spline_st)
```

**Step 4:** The convergence of the algorithm is examined and he parameter estimates from the model can be investigated using the following:
```{r,summaryspst, eval = FALSE}
...
```{r,plotspstres, eval = FALSE,fig.align = 'center',fig.width = 7,fig.height = 5}
plot(jags_output.ni_spline_st)
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% uncertainty interval. In order to view the rate of change plot, the following setting should be used:

```{r,plotspstresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_spline_st,plot_type = "rate_plot")
```
This will print the plot of the rate of change with 95 \% uncertainty intervals. 

To examine the data creating these plots, can be accessed by the user:
```{r,outdfspst, eval = FALSE}
...
This model needs an adequate number of sites to perform the decomposition and the minimum sites required will depend on the signal of the data. We found that in general we need a minimum of ?? sites. **Need to check this but the knots things are still a problem**
...
**Step 3a:** A brief insight into the outputs of the `reslr_output` function can be obtained using:
```{r,printnigamout, eval=FALSE}
print(jags_output.ni_gam_decomp)
```

**Step 4:** The convergence of the algorithm is examined and he parameter estimates from the model can be investigated using the following:
```{r,summarynigam, eval = FALSE}
...
```{r,plotnigamres, eval = FALSE,fig.align = 'center',fig.width = 7,fig.height = 5}
plot(jags_output.ni_gam_decomp,plot_tide_gauge = FALSE)
```
In addition, the user can select to plot the additional tide gauge data, `plot_tide_gauge = TRUE` in the plot. The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% uncertainty interval. 
In order to view the rate of change plot, the following setting should be used:

```{r,plotnigamresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "rate_plot")
```
This will print the plot of the rate of change with 95 \% uncertainty intervals. To examine the data creating the total model fit result plot, the user can use:
```{r,totaldf, eval = FALSE}
...
```{r,plotnigamregres, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "regional_plot")
```
The regional component and the rate of change of the regional component is presented with 95\% Uncertainty interval. To examine the data creating the regional component plot, the user can use:
```{r,regdf, eval = FALSE}
...
```{r,plotnigamregresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "regional_rate_plot")
```
Similarly, the rate of change of the regional component is presented with 95\% Uncertainty interval. To examine the data creating the regional component rate of change plot the user can use:
...
```{r,plotnigamlinres, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "linear_local_plot")
```
The linear local component is plotted with 95\% Uncertainty interval. To examine the data creating the linear local component plot, the user can use:
...
```{r,plotnigamnonlinres, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "non_linear_local_plot")
```
The non-linear local component is plotted with with 95\% Uncertainty interval. To examine the data creating the non-linear local component plot, the user can use:
...
```{r,plotnigamnonlinresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "non_linear_local_rate_plot")
```
The rate of change of the non-linear local component is plotted with with 95\% uncertainty interval. To examine the data creating the non-linear local component plot rate of change plot the user can use:
...
```{r,plotnigamall, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(jags_output.ni_gam_decomp,plot_type = "nigam_component_plot")
```
Each component is plotted with an 95\% uncertainty interval and this plot gives insight into the variability of the different components through time and at the different locations in question.
...
multi_site <- reslr::reslr_load(data = multi_site,
                                ...
                                prediction_interval = 20)
```
Here is a plot of the output example:
