...
- Longitude is the longitude of the site in degrees (in decimal, range: -180 to 180). It is important to note that if there is slightly different longitude values for all the observations in site this will lead to errors. Therefore, select one longitude value for each site.

- Latitude is the Latitude of the site in degrees (degree in decimal, range -90 to 90). Similarly, it is important to note that if there is slightly different latitude values for all the observations in site this will lead to errors. Therefore, select one latitude value for each site.
...
**Step 1.** Load in the data using `reslr_load`. If tide gauge data required update `include_tide_gauge = TRUE`. The user can select the resolution of the output by changing the value of `n_prediction = 100` with the default of 100 years. The `input_Age_type` argument is associated with the type of input Age where the default is in Common Era. 

**Step 2.** Plot the raw data using `plot`

**Step 3.** Choose your preferred model type from the available list above and use the `reslr_mcmc` function to obtain the parameter estimates
...
CedarIslandNC <- reslr::reslr_load(data = CedarIslandNC,
                            ...
                            input_Age_type = "CE")
```
In this function, the user can select to add tide gauge data by changing `include_tide_gauge = TRUE`. Note that for a simple linear regression we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `n_prediction` with the default set at 100 years. 
...
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.eiv_slr_t)
```
...
params$par_summary
```
The output of this function allows to user to examine the parameter estimates. For the eiv_slr_t model, the parameters of interest are the intercept (alpha), the slope (beta) and the standard deviation of the model **How I explain this easily** (sigma_res). When using the eiv_slr_t model, an estimate of the of the rate of sea-level change can be obtained by examining the value of the slope, i.e. beta. In this example, the rate of sea-level change for Cedar Island North Carolina using the EIV simple linear regression model is estimated to be XXXX.
...
CedarIslandNC <- reslr::reslr_load(data = CedarIslandNC,
                            ...
                            n_prediction = 100)
```
In this function, the user can select to add tide gauge data by changing `include_tide_gauge = TRUE`. Note that for this model we recommend using the default settings as demonstrated in the above code chunk. The output of this function the the updated dataframe which will be used in the following steps.
...
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.eiv_cp1_t)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 
...
params$par_summary
```
For the eiv_cp_t model, the parameters of interest are the intercept (alpha), the slopes before the change point (beta[1]) and after the change point (beta[2]), the year of the change point(cp) and sigma_res the variance of the model (**Again bad description here**).
...
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.eiv_igp_t)
```
...
params$par_summary
```
The length scale parameter, "phi" determines the distance over which two input points can influence each other. More specifically, it determines the rate at which the correlation between the input data points decays as the distance between those points increases. "sigma_res".... **FINISH**
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.ni_spline_t)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 
...
params$par_summary
```
In this situation, we can present the standard deviation associated with the NI spline space time model. Where "sigma_r" highlights the variation associated with the temporal model and "sigma_res" presenting the overall variation.
...
multi_site <- reslr::reslr_load(data = multi_site,
                            ...
                            n_prediction = 100)
```
In this function, the user can select to add tide gauge data by changing `include_tide_gauge = TRUE`.**FINISH** Note that for this model we recommend using the default settings as demonstrated in the above code chunk. The output of this function the the updated dataframe which will be used in the following steps.
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.ni_spline_st)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 
...
params$par_summary
```
In this situation, we can present the standard deviation associated with the NI spline space time model. Where "sigma_l" highlights the variation associated with the spatial temporal model and "sigma_res" presenting the overall variation.
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = TRUE}
reslr::check_convergence(object = jags_output.ni_gam_decomp)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 
...
The linear local component is plotted with 95\% Uncertainty interval. The non-linear local component is plotted with with 95\% Uncertainty interval. The associated linear local component rates can be accessed by: 
**FINISH THIS**
