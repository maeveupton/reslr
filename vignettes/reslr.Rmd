...
The tide gauges undergo a cleaning process within the package. Firstly, certain tide gauges have been flagged by the PSMSL website and have been removed. Secondly, each tide gauge data record must have data longer than 20 years and if this is not the case, the tide gauge is removed. The output will be decadaly averaged tide gauge data. 

Within the `reslr_load` function, the user has three options to choose from:
1. Provide a list of the preferred tide gauges from the [PSMSL website](https://psmsl.org/data/obtaining/), ensuring spelling, captailisation and spacing is exactly the same as the website. Note, the package will not work if error in spelling occurs. In addition, certain tide gauges have been flagged by the PSMSL website and are not included in this package and will return an error if selected.

2. The nearest tide gauge to proxy site based on minimum distance in kilometers 

3. Any tide gauge within 1 degree from the proxy site.
...
If tide gauge data required update the argument `include_tide_gauge = TRUE`, from this the user is request to choose whether to provide the list of names for the tide gauges from PSMSL website that the wish to use, `list_preferred_TGs`, or let the package find the tide gauge closest to the proxy site `TG_minimum_dist_proxy`. 
