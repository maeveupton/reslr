...
1. Provide a list of the preferred tide gauges from the [PSMSL website](https://psmsl.org/data/obtaining/), ensuring spelling, captailisation and spacing is exactly the same as the website. Note, the package will not work if error in spelling occurs. In addition, certain tide gauges have been flagged by the PSMSL website and are not included in this package and will return an error if selected. This is done by giving a list to the `list_preferred_TGs = c("ARGENTIA)` option in the `reslr_mcmc` function
...
The user can select the resolution of the output by changing the value of `prediction_interval = 100` with the default of 100 years. 
...
Note that for a simple linear regression we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 100 years. 
...
Note that for a change point model, we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 100 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
...
Note that for an IGP we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 100 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
...
Note that for a spline in time, we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 100 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
- The `data` dataframe is the inputted data with additional columns for the data_type_id which will contain "ProxyRecord". It can be accessed by:
```{r,dataspt, eval=TRUE}
data <- CedarIslandNC$data
...
```
Each component is plotted with an 95\% uncertainty interval and this plot gives insight into the variablity of the different components through time and at the different locations in question.
## Advanced 
...
In the package the user has the ability accesss all the posterior sample for each unknown parameter in the following way:
```{r, eval = FALSE}
# Example
