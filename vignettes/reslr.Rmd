...
vignette: >
  %\VignetteIndexEntry{reslr: Statistical Models for examining Relative Sea Level Change in R}
...
If you require fast instructions, check out the reslr: [Quick start](https://maeveupton.github.io/reslr/articles/quick_start.html).
...
| Noisy Input spline in time | High: A non-linear fit using regression splines using the method of Upton et al (2023). | **"ni_spline_t"** |
...
As presented in this table, users can choose from seven statistical models with varying complexities. The simplest is the errors-in-variables simple linear regression, suitable for examining linear trends in sea level data but limited to linear trends and a single location. The errors-in-variables change point model requires users to specify 1, 2, or 3 change points, making it effective for identifying abrupt changes and estimating change rates but limited to a few change points and a single location.

Moving to higher complexity, the errors-in-variables integrated Gaussian Process offers advantages over previous models. It is flexible in time, providing sea level trends and associated rates of change, though confined to a single location with long computational run times. For improved computational efficiency, the noisy input spline in time maintains advantages of the errors-in-variables integrated Gaussian process but is restricted to a single location.

To explore multiple locations in time and space, the noisy input spline in space and time captures sea level trends and rates for many locations simultaneously. The final approach, the noisy input generalised additive model for decomposing the RSL signal. Users need to provide additional information on the physical processes driving RSL changes in space and time, offering insights into these underlying mechanisms.
...
- Age is the year of the data in Common era ("CE") or Before Present ("BP"). If calibrated in the BP form the package will convert the data into the Common Era form.
...
If tide gauge data is required update the argument `include_tide_gauge = TRUE`, from this the user has three options as described above. First, provide the list of names for the tide gauges from PSMSL website that the wish to use in the `list_preferred_TGs` option. Second, the package find the tide gauge closest to the proxy site `TG_minimum_dist_proxy = TRUE`. Third, the package uses all tide gauges within 1 degree of the proxy site `all_TG_1deg = TRUE`. If sedimentation accumulation rates for the proxy records are less than or greater than a decade the user can alter this size using `sediment_average_TG = 10` which has a default of 10 years.
...
The `input_age_type` argument is associated with the type of input age where the default is in Common Era. The package can model Before Present by updating this setting to "BP" and more information is in the advanced vignette. 
...
```{r,dataigp,eval=FALSE}
data <- CedarIslandNC_input$data
```
- The `data_grid` is a dataframe that is evenly spaced in time based on the `prediction_grid_res` value chosen by the user and is used to create the plots and is accessed by
```{r,datagridigp, eval = FALSE}
data_grid <- CedarIslandNC_input$data_grid
```
...
**Step 3a:** A brief insight into the outputs of the `reslr_output` function can be obtained using:
```{r,printigpout, eval=FALSE}
print(res_eiv_igp_t)
```

**Step 4:** The convergence of the algorithm is examined and he parameter estimates from the model can be investigated using the following:
```{r,summaryigp, eval = FALSE}
...
```{r,plotigpres, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(res_eiv_igp_t,
...
```
```{r,plotigpres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotigpresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(res_eiv_igp_t,
  plot_type = "rate_plot",
  xlab = "Year (CE)",
...
```{r,plotigpresrate_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
An alternative method to examine how the response variable varies in time is using the Noisy input spline in time (ni_spline_t). 
...
```{r,runspt,eval = TRUE,message=FALSE,results='hide'}
...
The Noisy input spline in space time examines changes in RSL over multiple locations and throughout time. 
...
**Step 2:** Plotting the data the raw data with:
```{r,plotspst,fig.align = 'center',fig.width = 7,fig.height = 10,eval = TRUE}
plot(
  x = multi_site_input,
  title = "Plot of the raw data",
  xlab = "Year (CE)",
...
```{r,runspst,eval = FALSE,results='hide',message=FALSE}
...
```{r,plotspstres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
url <- "https://raw.githubusercontent.com/maeveupton/reslr/main/reslrvigplots/plotspstres-1.png"
knitr::include_graphics(url)
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% credible interval. The caption provides the model type used and number of proxy sites and tide gauge sites used and can be removed if necessary with `plot_caption = FALSE`. In order to view the rate of change plot, the following setting should be used:

```{r,plotspstresrate, fig.align = 'center',fig.width = 7,fig.height = 5,eval =FALSE,results='hide',message=FALSE}
plot(res_ni_spline_st,
  plot_type = "rate_plot",
  xlab = "Year (CE)",
  y_rate_lab = "Rate of Change (mm per year)"
...
```{r,plotspstresrate_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
**Step 1:** Load in the data using the `reslr_load` function: 
```{r,loadnigam, eval = FALSE}
multi_9_sites_input <- reslr::reslr_load(
  data = multi_9_sites,
...
The output of this function is a list of two dataframes called `data` and `data_grid`. 
- The `data` dataframe is the inputted data with additional columns for the linear_rate, linear_rate_err and data_type_id which will contain two options, "ProxyRecord" or "TideGaugeData". It can be accessed by:
```{r,datanigam,eval=FALSE}
data <- multi_9_sites_input$data
```
- The `data_grid` is a dataframe that is evenly spaced in time based on the `prediction_grid_res` value chosen by the user and is used to create the plots and is accessed by
```{r,datagridnigam, eval = FALSE}
data_grid <- multi_9_sites_input$data_grid
```

**Step 1a:** A brief insight into the outputs of the `reslr_input` function can be obtained using:
```{r,printnigam, eval=FALSE}
print(multi_9_sites_input)
```

**Step 2:** Plotting the data the raw data with:
```{r, plotnigam,fig.align = 'center',fig.width = 7,fig.height = 10,eval = FALSE}
plot(
...
```
```{r,plotnigamdata, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
url <- "https://raw.githubusercontent.com/maeveupton/reslr/main/reslrvigplots/plotnigam-1.png"
knitr::include_graphics(url)
```
This will produce a plot of Age on the x-axis and Relative Sea Level on the y-axis in meters. Grey boxes represent the uncertainty associated with the vertical and horizontal uncertainty. The black data points are the midpoints of these uncertainty boxes. The separate sites will appear in separate windows on the plot. The extra arguments can be used which allows the user to updated the titles and axis labels. The caption `plot_caption`, included by default, provides the number of proxy sites and tide gauge sites that will be used in the model and can be removed if required `plot_caption = FALSE`. In addition, the user can select to plot the additional tide gauge data, `plot_tide_gauge = TRUE`.

**Step 3:** Run the model
```{r,runnigam,eval = FALSE,results='hide',message=FALSE}
res_ni_gam_decomp <- reslr::reslr_mcmc(
...
```{r,plotnigamres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamresrate_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamregres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamregresrate, fig.align = 'center',eval = FALSE}
plot(res_ni_gam_decomp, plot_type = "regional_rate_plot")
```
```{r,plotnigamregresrate_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamlinres, fig.align = 'center',eval = FALSE}
plot(res_ni_gam_decomp, plot_type = "linear_local_plot")
```

```{r,plotnigamlinres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamnonlinres_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamnonlinresrate_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
```{r,plotnigamall, fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE,message=FALSE,results='hide'}
plot(res_ni_gam_decomp, plot_type = "nigam_component_plot")
```

```{r,plotnigamall_load, eval = TRUE,echo=FALSE, fig.align = 'center',out.width="100%"}
...
Aarup, T., M. Merrifield, B. Pérez Gómez, I. Vassie, and P.Woodworth. 2006. “Manual on
Sea-level Measurements and Interpretation, Volume IV : An update to 2006.” Intergovernmental
Oceanographic Commission of UNESCO 4. https://unesdoc.unesco.org/ark:/48223/pf0000147773
