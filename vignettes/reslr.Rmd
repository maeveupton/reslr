...
**Step 1.** Load in the data using `reslr_load`. If tide gauge data required update `tide_gauge_included = TRUE`. The user can select the resolution of the output by changing the value of `n_prediction = 100`. The default of 100 years.
...
```{r,eval = TRUE}
jags_output.ni_spline_st <- reslr::reslr_mcmc(input_data =  multi_site, model_type = "ni_spline_st")
```
