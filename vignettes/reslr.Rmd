...
The tide gauges undergo a cleaning process within the package. Firstly, certain tide gauges have been flagged by the PSMSL website and have been removed. Secondly, each tide gauge data record must have data longer than 20 years and if this is not the case, the tide gauge is removed. The output will be tide gauge data averaged over a decade. 

Within the `reslr_load` function, the user has three options to choose from:
1. Provide a list of the preferred tide gauges from the [PSMSL website](https://psmsl.org/data/obtaining/), ensuring spelling, captailisation and spacing is exactly the same as the website. Note, the package will not work if error in spelling occurs. In addition, certain tide gauges have been flagged by the PSMSL website and are not included in this package and will return an error if selected. This is done by giving a using the `list_preferred_TGs` opt
...
**Step 2:** Plotting the data the raw data with:
```{r,plotdatacp,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
...
**Step 2:** Plotting the data the raw data with:
```{r, plotigpdata,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(x =  CedarIslandNC)
```
This will produce a plot of Age on the x-axis and Relative Sea Level on the y-axis in meters. Grey boxes represent the uncertainty associated with the vertical and horizontal uncertainty. The black data points are the midpoints of these uncertainty boxes. To alter the further plot, the following extra arguments can be used which allows the user to updated the titles and axis labels. In addition, the user can select to plot the additional tide gauge data, `plot_tide_gauge = TRUE` in the plot:
...
**Step 2:** Plotting the data the raw data with:
```{r,plotspt,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(x =  CedarIslandNC)
```
This will produce a plot of Age on the x-axis and Relative Sea Level on the y-axis in meters. Grey boxes represent the uncertainty associated with the vertical and horizontal uncertainty. The black data points are the midpoints of these uncertainty boxes. To alter the further plot, the following extra arguments can be used which allows the user to updated the titles and axis labels. In addition, the user can select to plot the additional tide gauge data, `plot_tide_gauge = TRUE` in the plot:
