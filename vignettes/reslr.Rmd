...
If tide gauge data required update the argument `include_tide_gauge = TRUE`, from this the user has three options as described above. Firstly, provide the list of names for the tide gauges from PSMSL website that the wish to use in the `list_preferred_TGs` option. Secondly, the package find the tide gauge closest to the proxy site `TG_minimum_dist_proxy = TRUE`. Thirdly, the package uses all tide gauges within 1 degree of the proxy site `all_TG_1deg = TRUE`. If sedimentation accumulation rates for the proxy records are less than or greater than a decade the user can alter this size of the rolling window average using `rolling_window_average = 10` which has a default of 10 years.
If linear_rate is of interest to the user update the argument `include_linear_rate = TRUE`. 
The user can select the resolution of the output by changing the value of `prediction_interval = 50` with the default of 50 years. 
...
```
In this function, the user can select to add tide gauge data and estimates for linear_rate, by changing `include_tide_gauge = TRUE` and `include_linear_rate = TRUE` respectfully. If `include_tide_gauge = TRUE` the user must decide if they require the closest tide gauge i.e. `TG_minimum_dist_proxy = TRUE`, or select specific tide gauge i.e. `list_preferred_TGs = c("ARGENTIA")`, or all tide gauges within 1 degree of the proxy site i.e. `all_TG_1deg = TRUE`. The default setting is `rolling_window_average = 10` which corresponds to sediment accumulation rates of the proxy records, yet the user has the ability to alter this sediment accumulation rate by changing the size of the rolling window average.
Note that for a simple linear regression we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 50 years. 
...
```
- The `data_grid` is a dataframe that is evenly spaced in time based on the `prediction_interval` value chosen by the user and is used to create the plots and is accessed by
...
```
This will produce a plot of Age on the x-axis and Relative Sea Level on the y-axis in meters. Grey boxes represent the uncertainty associated with the vertical and horizontal uncertainty. The black data points are the midpoints of these uncertainty boxes. To alter the further plot, the following extra arguments can be used which allows the user to updated the titles and axis labels. The caption provides the number of proxy sites and tide gauge sites that will be used in the model. In addition, the user can select to plot the additional tide gauge data, `plot_tide_gauge = TRUE` in the plot:

```{r,plotdataextended,fig.align = 'center',fig.width = 7,fig.height = 5,eval = TRUE}
plot(
...
```
This command takes the input data and the user specifies the statistical model, i.e. a simple linear regression using the EIV uncertainty method ("eiv_slr_t"). The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. The function tells `reslr` to store the output of the model run in an object called `res.eiv_slr_t`.
...
plot(res.eiv_slr_t)
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% credible interval. The caption provides the model type used and number of proxy sites and tide gauge sites used.
...
Note that for a change point model, we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 50 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
...
```{r,plotdatacpmore,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(
  x =  CedarIslandNC_input,
  title = "Plot of the raw data",
  xlab = "Age (CE)",
...
```
The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. Similar to the earlier model, the output object `res.eiv_cp1_t` stores the JAGS model run and should take a second to run.
...
plot(res.eiv_cp1_t)
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% credible interval. The caption provides the model type used and number of proxy sites and tide gauge sites that will be used.
...
Note that for an IGP we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 50 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
...
```{r, plotigpdatamore,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(
  x =  CedarIslandNC_input,
  title = "Plot of the raw data",
  xlab = "Age (CE)",
...
```
This command takes the input data and the user specifies the statistical model, i.e. an integrated Gaussian process using the EIV uncertainty method ("eiv_slr_t"). It tells `reslr` to store the output of the model run in an object called `res.eiv_igp_t`. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. The computational run time for this model is approximately 14 minutes.
...
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. In order to view the rate of change plot, the following setting should be used:
...
Note that for a spline in time, we recommend using the default settings as demonstrated in the above code chunk. The user can alter the resolution of the output plots using `prediction_interval` with the default set at 50 years. 
The output of this function is a list of two dataframes called `data` and `data_grid`. 
- The `data` dataframe is the inputted data with additional columns for the data_type_id which will contain "ProxyRecord". It can be accessed by:
```{r,dataspt, eval=TRUE}
data <- CedarIslandNC_input$data
...
```{r,plotsptextra,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
plot(
  x =  CedarIslandNC_input,
  title = "Plot of the raw data",
  xlab = "Age (CE)",
...
res.ni_spline_t <- reslr_mcmc(
  ...
  CI = "95%")
```
The output object `res.ni_spline_t` stores the JAGS model run. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. Note that there will be two model runs printed in the console here but the output will be the same format as earlier models. 
...
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% credible interval. The caption provides the model type, number of proxy sites and tide gauge sites that were used. In order to view the rate of change plot, the following setting should be used:
...
For this model, a minimum of 2 proxy sites should be used and tide gauge data provides insight into recent changes in RSL if the user is requires. As an example, we will filter the example dataset `NAACproxydata` to select two sites to demonstrate the process:
...
```{r,plotspstmore,fig.align = 'center',fig.width = 7,fig.height = 5,eval = TRUE}
plot(
  x =  multi_site_input,
  title = "Plot of the raw data",
  xlab = "Age (CE)",
...
res.ni_spline_st <- reslr_mcmc(
  ...
  CI = "95%")
```
The output object `jags_output.ni_spline_st` stores the JAGS model run. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. Note that additional computational run time is required for the model compared with the ni_spline_t. 
...
This model needs an adequate number of proxy sites to perform the decomposition and the minimum sites required will depend on the signal of the data. We found that in general we need a minimum of ?? proxy sites. Also, we strongly recommend using tide gauge data for this model to obtain insight into recent changes in RSL. **Need to check this but the knots things are still a problem**
...
multi_8_sites_input <- reslr_load(data = multi_8_sites,
                                ...
                                rolling_window_average = 10)
```
In this function, the user can select to add tide gauge data and estimates for linear_rate, by changing `include_tide_gauge = TRUE` and `include_linear_rate = TRUE` respectfully. If `include_tide_gauge = TRUE` the user must decide if they require the closest tide gauge i.e. `TG_minimum_dist_proxy = TRUE`, or select specific tide gauge i.e. `list_preferred_TGs = c("ARGENTIA")`, or all tide gauges within 1 degree of the proxy site i.e. `all_TG_1deg = TRUE`. In this example, we use all tide gauges within 1 degree of the proxy site. The default setting is `rolling_window_average = 10` which corresponds to sediment accumulation rates of the proxy records, yet the user has the ability to alter this sediment accumulation rate by changing the size of the rolling window average.
...
```{r,plotnigammore, fig.align = 'center',fig.width = 7,fig.height = 5,eval = TRUE}
plot(
...
```
The output object `res.ni_gam_decomp` stores the JAGS model run. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is `CI = "95%"`. Note that there will be two model runs printed in the console here but the output will be the same format as earlier models. 
...
```
The regional component and the rate of change of the regional component is presented with 95\% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. To examine the data creating the regional component plot, the user can use:
```{r,regdf, eval = FALSE}
...
```
Similarly, the rate of change of the regional component is presented with 95\% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. To examine the data creating the regional component rate of change plot the user can use:
...
```
The linear local component is plotted with 95\% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. To examine the data creating the linear local component plot, the user can use:
...
```
The non-linear local component is plotted with with 95\% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. To examine the data creating the non-linear local component plot, the user can use:
...
```
The rate of change of the non-linear local component is plotted with with 95\% credible interval. The caption provides the model type, the number of proxy sites and tide gauge sites that were used. 
...
```
Each component is plotted with an 95\% credible interval and this plot gives insight into the variability of the different components through time and at the different locations in question. The caption provides the model type, the number of proxy sites and tide gauge sites that were used.
...
Whitehouse, Pippa L (2018), ‘Glacial isostatic adjustment modelling: historical perspectives, recent
advances, and future directions ’, Earth Surf. Dynam 6, 401–429.
...
Pugh, David, and Philip Woodworth. 2014. “Tidal Forces: Sea-Level Science: Understanding Tides,
Surges, Tsunamis and Mean Sea-Level Changes.” In Sea-Level Science: Understanding Tides, Surges,
Tsunamis and Mean Sea-Level Changes, 36–59. Cambridge University Press. 
...
“New Data Systems and Products at the Permanent Service for Mean Sea Level.” Journal of Coastal
Research 29 (3): 493–504. 
