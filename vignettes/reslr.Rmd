...
```
The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. Similar to the earlier model, the output object `res_eiv_cp1_t` stores the JAGS model run and should take a second to run.
...
```
For the parameter estimates, we can present the standard deviation associated with the NI spline time model. Where "sigma_r" highlights the variation associated with the spline coefficient for the spline in time and "sigma_res" presenting the overall variation of the model.
...
res_ni_spline_st <- reslr_mcmc(
  ...
  CI = "95%")
```
The output object `jags_output.ni_spline_st` stores the JAGS model run. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is ` CI = "95%"`. Note that additional computational run time is required for this model compared with the ni_spline_t. 
...
```
For the parameter estimates, we can present the standard deviation associated with the NI spline space time model. Where "sigma_l" highlights the variation associated with the spline coefficient of the spline in time and "sigma_res" presenting the overall variation.
...
This model needs an adequate number of proxy sites to perform the decomposition and the minimum sites required will depend on the signal of the data. We found that in general we need a minimum of 5 proxy sites and at least 5 associated tide gauge sites. Also, we strongly recommend using tide gauge data for this model to obtain insight into recent changes in RSL. 
...
```
The output object `res_ni_gam_decomp` stores the JAGS model run. The `CI` setting allows the user to set the credible intervals from a list of the following 50\%, 95\% or 99\%, the current default is `CI = "95%"`. Note that there will be two model runs printed in the console here but the output will be the same format as earlier models. 
