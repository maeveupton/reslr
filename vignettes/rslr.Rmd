---
title: "Models for examining Relative Sea Level Change in R with rslr"
author: "Maeve Upton"
date: "`r Sys.Date()`"
output: 
...
## Installation of the `rslr` package

The `rslr` package uses the JAGS (Just Another Gibbs Sampler) software to run the models. Before installing `rslr`, visit the [JAGS](https://sourceforge.net/projects/mcmc-jags/) website and download and install JAGS for your operating system. 


Next, start Rstudio and find the window with the command prompt (the symbol `>`). Type

```{r,eval = FALSE}
install.packages("rslr")
```
It may ask you to pick your nearest CRAN mirror (the nearest site which hosts R packages). You will then see some activity on the screen as the `rslr` package and the other packages it uses are downloaded. The final line should then read: 

`package 'rslr' successfully unpacked and MD5 sums checked`
...
This will load the `rslr` package and all the associated packages. You’ll need to type the `library(rslr)` command every time you start R. If you have problems, visit the [Issues](https://github.com/maeveupton/rslr/issues) page and leave a message to tell us what went wrong. 

## Considerations before running rslr

Prior to running the `rslr` package, there are a few points to consider.
...
```{r,eval = FALSE}
# For 1 site
data_1site <- rslr::NAACproxydata %>% dplyr::filter(Site == "Cedar Island")
# For multiple sites
data_multisite <- rslr::NAACproxydata %>% dplyr::filter(Site %in% c(
...
`rslr` can handle three different types of data structure. It is important to note that varying the number of data sites will require different statistical modelling strategy:
...
- Age is the year of the data in Common era. Ensure the Age is calibrated in this form. If conversions are required complete before using the package, e.g. CE = Age - 1950. *CHECK*
...
The general structure for running `rslr` is as follows:

**Step 1.** Load in the data using `load_input_data`. If tide gauge data required update `tide_gauge_included = TRUE`

**Step 2.** Plot the raw data using `plot_data`
...
**Step 6.** Visualise the results with `plot_result`


## Errors-in-Variables Simple Linear Regression (**"eiv_slr_t"**)

The simplest model the `rslr` package can fit is a simple linear regression using the Errors-in-Variables method to account for the uncertainty associated with the proxy records, i.e. uncertainty associated with input (age) and the output (RSL). We would not recommend any model simpler than this (e.g. `lm`) as it will ignore some of the key uncertainties in the data. 
...
```{r, eval = FALSE}
example_data_set <- rslr::NAACproxydata 
# For 1 site
CedarIslandNC <- example_data_set %>% dplyr::filter(SiteName == "Cedar Island,\n North Carolina")
```

**Step 1:** Load in the data using the `load_input_data` function: 
```{r, eval = FALSE}
CedarIslandNC <- rslr::load_input_data(data = CedarIslandNC,
...
rslr::plot_data(input_data = CedarIslandNC)
```
...
```{r,eval = FALSE}
jags_output.eiv_slr_t <- rslr::run_mcmc(
  input_data = CedarIslandNC,
  model_type = "eiv_slr_t"
)
```
This command takes the input data and the user specifies the statistical model, i.e. a simple linear regression using the EIV uncertainty method ("eiv_slr_t"). It tells `rslr` to store the output of the model run in an object called `jags_ouput.eiv_slr_t`.


**Step 4:** To examine if the algorithm has converged use:
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.eiv_slr_t)
```
...
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.eiv_slr_t)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.eiv_slr_t)
```
...
**Step 1:** Load in the data using the `load_input_data` function: 
```{r, eval = FALSE}
CedarIslandNC <- rslr::load_input_data(input_data = CedarIslandNC,
                            tide_gauge_included = FALSE)
```
...
```{r eval = FALSE}
jags_output.eiv_cp1_t <- rslr::run_mcmc(
  input_data  = CedarIslandNC,
...
```{r eval = FALSE}
jags_output.eiv_cp2_t <- rslr::run_mcmc(
  input_data = CedarIslandNC,
...
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.eiv_cp1_t)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 

**Step 5:** the parameter estimates from the model can be investigated:
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.eiv_cp1_t)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.eiv_cp1_t)
```
The output of this function is a graph of the input data, i.e. Age and RSL and associated uncertainty boxes, and the model fit with 95 \% uncertainty interval.
...
```{r eval = FALSE}
jags_output.eiv_igp_t <- rslr::run_mcmc(
  input_data = CedarIslandNC,
  model_type = "eiv_igp_t"
)
```
This command takes the input data and the user specifies the statistical model, i.e. an integrated Gaussian process using the EIV uncertainty method ("eiv_slr_t"). It tells `rslr` to store the output of the model run in an object called `jags_output.eiv_igp_t`. The computational run time for this model is long **XXX minutes??**


**Step 4:** the convergence of the algorithm is inspected using:
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.eiv_igp_t)
```
...
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.eiv_igp_t)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.eiv_igp_t)
```
...
```{r,eval = FALSE}
jags_output.ni_spline_t <- rslr::run_mcmc(input_data = CedarIslandNC,
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.ni_spline_t)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 

**Step 5:** the parameter estimates from the model can be investigated:
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.ni_spline_t)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.ni_spline_t)
```
...
example_data_set <- rslr::NAACproxydata
# For 2 site
multi_site <- example_data_set %>% dplyr::filter(SiteName %in% c("Cedar Island","Nassau"))
```

**Step 1:** Load in the data using the `load_input_data` function: 
```{r, eval = FALSE}
multi_site <- rslr::load_input_data(input_data = multi_site,
                            tide_gauge_included = FALSE)
```
...
**Step 2:** Plotting the data the raw data with:
```{r,fig.align = 'center',fig.width = 7,fig.height = 5,eval = FALSE}
rslr::plot_data(input_data = multi_site)
```
...
```{r,eval = FALSE}
jags_output.ni_spline_st <- rslr::run_mcmc(input_data = multi_site, model_type = "ni_spline_st")
```
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.ni_spline_st)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 

**Step 5:** The parameter estimates from the model can be investigated:
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.ni_spline_st)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.ni_spline_st)
```
...
If the GIA rate is not provided for each location, then the `rslr` package will calculate it using the data and if this is not possible, the package will print an error message.
...
```{r,eval = FALSE}
jags_output.ni_gam_decomp <- rslr::run_mcmc(
  input_data = multi_site,
...
**Step 4:** The convergence of the algorithm is examined using the following:
```{r eval = FALSE}
rslr::check_convergence(jags_output = jags_output.ni_gam_decomp)
```
If the model run has converged properly the values should be close to 1. If they are above 1.1, we recommend a longer run. 

**Step 5:** The parameter estimates from the model can be investigated:
```{r eval = FALSE}
params <- rslr::parameter_estimate(jags_output = jags_output.ni_gam_decomp)
params$par_summary
```
...
```{r eval = FALSE}
rslr::plot_result(jags_output = jags_output.ni_gam_decomp)
```
...
```{r,eval = FALSE}
output_dataframes <- parameter_estimate(jags_output = run_mcmc(data = CedarIslandNC, model_type = "ni_spline_t"))
...
```{r, eval = FALSE}
# Example
final_plots <- plot_result(jags_output = run_mcmc(data = CedarIslandNC, model_type = "ni_spline_t"))
